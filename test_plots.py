...
# get the actual VICON data for rotations

# get the estimated rotation quaternions

# add subplots for each wrt to the unix timestamps to compare on a single plot
...
if __name__ == "__main__":
    ...
    for seq_id in tqdm(seq_ids):
        ...
        f1 = open('./test_pickles/estimate_'+seq_id+'_9','rb')
        tr = pickle.load(f1)
        f1.close()
        print(f"tr {tr.shape}")
        ...
        plt.plot(tr_eul[:,0])
        plt.plot(f_tr_eul[:,0])
        plt.legend(['OPTMIZED','MOTIONMODEL'])
        # plt.show()
